...
Before the installation of Ceph, there are additional task to carry out on Google Cloud. Create a empty disk for each node. The empty disk will be used for the Ceph setup.

Additionally, create another VPC network (NIC 1) and set a IP range of your choice. Set the region to the same region as the VM instance, in this case is 'asia-southeast1'. Apply the same set of firewall rules as the 'default' VPC network setting.
...
Piror to the creating of inner VM, create a virtual bridge for the inner VM. Use any ip address of your preference.
The table below shows the ip address that i used for this configuration.
...
Edit `/etc/network/interfaces` and edit/add in the following lines:
```
...
```
The lines that were added allow the virtual bridge to be route to the eth0, where Internet access is available.

The `/etc/network/interfaces` file now looks like this:
```
